...
def change_file(old_value,new_value):
  ...
  with open("tb_fo4.spice", "r") as file:
    ...
    for i, line in enumerate(lines):
      ...
      if "write data_"+old_value+".raw wnv wpv trv tfv" in line:
        lines[i] = line.replace("data_"+old_value+".raw", "data_"+new_value+".raw")

  with open("tb_fo4.spice", "w") as file:
...
for i in range(len(corners)): # runs over the 18 corners to apply the function "change_file"
  if i>0:
    ...
    subprocess.run(["ngspice", "-b", "tb_fo4.spice"]) # run file
